...
- Focus on what needs to be verified, audited, or designed (e.g. *"Audit whether the implementation matches what the user requested and identify any unintended side effects"*).
- Do not spoon-feed file paths, function names, or lines of code you already touched. Let the subagent locate files, trace dependencies, and evaluate the solution independently.
...
- **No Pre-Barking Edge Cases (No "e.g.")**: Never supply examples of edge cases you already thought of and handled. The subagent has full domain intelligence; giving `(e.g., X, Y)` causes anchoring bias, prompting it to verify the cases you already solved while blinding it to the cases you overlooked. Require it to deduce failure modes from first principles.
...
### 2. Primary Objective
<The concrete deliverable: e.g., audit compliance against what the user asked for, detect regressions, or analyze system behavior>
...
### 2. The False-Elevation Trap (The Pseudo-Success)
...
### 3. Pre-Barking Edge Cases (Anchoring Bias)
- **What happens**: Supplying `(e.g., X, Y)` in the prompt, where X and Y are the exact scenarios the parent agent already thought of and coded for.
- **Why it fails**: Causes anchoring bias. The subagent expends its reasoning budget verifying the cases you already solved (zero new value) while remaining blind to unhandled edge cases Z and W that you never conceived.
- **Correct action**: Never list handled edge cases. Mandate that the subagent deduce all domain failure modes and boundary conditions from first principles.

### 4. Missing the Mandate (Deferential Rubber-Stamping)
...
### 5. Direct Artifact Path Assumption (Tooling Failure)
