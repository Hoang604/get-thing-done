...
- **Target Seam / Signature (`Literal Code`):** Write the exact literal code block for the interface/function signature (`e.g., def process_order(order: Order, payment_port: PaymentPort) -> OrderResult:`). Do not write prose summaries like `"Update function to handle Y"`.
- **Interface Invariants & Error Modes:** State exact invariants (`what must not change across execution`) and exact typed error modes (`exceptions raised or error return variants`).
- **Dependency Category & Adapter:** Explicitly classify (`In-process`, `Local-substitutable`, `Remote-owned`, or `True-external`) and specify the exact adapter, port, or stand-in used across the seam.
