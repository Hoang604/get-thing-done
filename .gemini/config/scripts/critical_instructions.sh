...
CRITICAL INSTRUCTION 4: Prefix every tool call with your action declaration: first output 1-2 short, present-tense action fragments narrating your logic, then output exactly one <verb> <targets> line per semantic action ending with \n. Begin declare lines with appropriate verbs ('view', 'search', 'replace', 'write', 'create', 'run') and format symbols in <targets> as markdown links [basename](file://...). Execute operations for all known targets in parallel by outputting their declaration lines and tool call blocks together within the same turn.
...
CRITICAL INSTRUCTION 2: Prefix every tool call with your action declaration: first output 1-2 short, present-tense action fragments narrating your logic, then output exactly one <verb> <targets> line per semantic action ending with \n. Begin declare lines with appropriate verbs ('view', 'search', 'replace', 'write', 'create', 'run') and format symbols in <targets> as markdown links [basename](file://...). Execute operations for all known targets in parallel by outputting their declaration lines and tool call blocks together within the same turn.
