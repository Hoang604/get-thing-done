...
- You must make your assumptions explicit before judging the code as correct or incorrect.
- At the beginning of the report, list **all material assumptions** you relied on.
- For each assumption, state:
  - the assumption itself
  - the basis for that assumption
  - whether the basis is **Observed** or **Inferred**
- Valid bases include:
  - explicit behavior visible in the scanned code
  - naming or API contract implied by the scanned code
  - user-provided objective, context, or focus areas
  - domain invariants that are necessary for the workflow to be considered correct
- Do not present an assumption as established truth if it is only inferred.
- Do not call behavior "wrong" until you have compared the code's actual behavior against the stated assumptions.
- If a finding depends on a weak or unverified assumption, say so directly in the finding and in residual uncertainty.
...
## 1.5. State Assumptions Before Conclusions

Before declaring any path correct or incorrect:

1. List the assumptions you need in order to evaluate behavior
2. State where each assumption comes from
3. Mark each assumption basis as **Observed** or **Inferred**
4. Carry those assumptions forward into each finding

If you cannot state the assumption clearly, do not overclaim the conclusion.
...
1. Identify which listed assumptions the finding depends on
2. State whether the finding is **Observed** or **Inferred**
3. Explain the triggering scenario
4. Describe what the code actually does
5. Explain why that behavior conflicts with the stated assumptions
6. Explain the likely impact
7. Suggest the smallest effective remediation that preserves invariants
...
- **A1:** {assumption}
- **Basis:** {where this assumption comes from}
...
- **What The Code Does:** {what the code actually does}
- **Why This Conflicts With The Assumptions:** {precise mismatch between the code path and the stated assumptions}
