...
1. **Domain Contract Integrity.** Every variable, parameter, and return value carries its exact domain contract. Model required domain properties as strictly non-nullable, reserving optionality exclusively for authorized semantic absence. Suppressing compiler or diagnostic feedback via untyped wildcards or escape hatches is strictly forbidden.
2. **Boundary Validation Membrane.** All data entering the system is untrusted by default. Raw boundary inputs must be transformed into strict domain types exclusively through validation, parsing, or type narrowing before reaching domain logic; unsafe type assertions bypassing runtime verification are strictly forbidden.
