...
Stop and wait for the user to answer the list. Assume every user reply is incomplete. You MUST recursively execute Step 1 on the user's new text to actively hunt for newly introduced unprovided paths, schemas, or edge cases. Output a new numbered list for every new `[MISSING]` tag found. You may only exit this loop when you explicitly declare `[ZERO_NEW_MISSING]`, proving no new variables exist.
